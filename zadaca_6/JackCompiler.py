...
class Compiler:
	...
	def _compileStatements(self):
		...
		while True:
			if self._tokenizer.tokenType() == TK_SYMBOL and self._tokenizer.symbol() == "}":
				break  
			keyword = self._expectKeyword((KW_DO, KW_IF, KW_WHILE, KW_LET, KW_RETURN, ))
			#TEST za _compileLet()
			if keyword == KW_LET:
				self._compileLet()
			#TEST za _compileDo()
				...
				self._compileDo()
			#TEST za _compileReturn()
			elif keyword == KW_RETURN:
				self._compileReturn()
			else:
				break
	...
	def _compileExpressionList(self): # DZ
		...
		"""
		#do draw(true, false, x+2);
		while True:
			if self._tokenizer.tokenType() == TK_SYMBOL and self._tokenizer.symbol() == ")":
				break

			print("new expression: ", self._tokenizer.tokenType())
			self._compileExpression()
			if self._tokenizer.tokenType() == TK_SYMBOL and self._tokenizer.symbol() != ",":
				break
