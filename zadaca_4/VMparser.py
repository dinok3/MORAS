class Parser:
    ...
    def _parseLines(self):
        ...
        for (line, n) in self._lines:
            ...
            if len(l) > 0:
                lines.append(l)
        if self._goto_labels != {}:
            for k, v in self._goto_labels.items():
                Parser._error("goto", v, f"Label({k}) Not Found")
            return False
    ...
    def _parseLine(self, line, n):
        ...
        if l[0] == "push":
            if len(l) == 3:
                if not l[2].isdigit():
                    self._flag = False
                    Parser._error("Parser", n, "Argument must be non-negative integer")
                    return ""
    ...
    def _label(self, lab, n):
        if lab in self._goto_labels:
            self._goto_labels.pop(lab, None)
        return "(" + self._func + "$" + lab + ")"
    
    def _goto(self, lab, n):
        self._goto_labels[lab] = n
        return "@" + self._func + "$" + lab + "\n0;JMP"
